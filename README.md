...
- **P2 (in progress):** the value-typed resolver stack — the `possible`-tier edges that make "report, don't rule" earn its keep. **Landed:** `self.m()` → the enclosing class's member (+ cross-file inherited); annotation-typed receivers (`u: User` → `User.save`, `MEDIUM`/possible); and **relative-import resolution** (`from ..model import X` now binds cross-module). **Next:** **package re-export resolution** (`from pkg import X` where `pkg/__init__` re-exports `X` from a submodule — needed for binding/annotation on re-export-heavy APIs, incl. this one); local inference (`x = User()`); heuristics; call-site tracing; `.pyi`/typeshed **stubs** (opt-in); and **resolution metrics** (coverage + confidence/source distribution, asserted in the golden eval).
