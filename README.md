...
- **P2 (in progress):** the value-typed resolver stack — the `possible`-tier edges that make "report, don't rule" earn its keep. **Landed:** `self.m()` → the enclosing class's member (+ cross-file inherited), the engine's first `possible`-tier edges (`MEDIUM`/inference — a subclass may override at runtime). **Next:** annotation-typed receivers (`u: User`), local inference (`x = User()`), heuristics, call-site tracing; `.pyi`/typeshed **stubs** (opt-in, scoped); and **resolution metrics** — coverage + confidence/source distribution, asserted in the golden eval.
