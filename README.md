...
- **P2 (in progress):** the value-typed resolver stack — the `possible`-tier edges that make "report, don't rule" earn its keep. **Landed:** `self.m()` → the enclosing class's member (+ cross-file inherited); annotation-typed receivers (`u: User` → `User.save`, `MEDIUM`/possible); **relative-import resolution** (`from ..model import X`); and **package re-export resolution** (`from pkg import X` follows `pkg/__init__` to the real defining module — this lit up annotation across the codebase). **Next:** local inference (`x = User()`); the **call-site resolver + confidence merge** (a call-site observation corroborating an annotation escalates the edge to `definite`); heuristics; `.pyi`/typeshed **stubs** (opt-in); and **resolution metrics** (coverage + confidence/source distribution, asserted in the golden eval).
