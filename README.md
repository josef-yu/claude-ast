...
- **P2:** the resolver stack — **value-typed** attribute/method calls (`obj.save()`, via local inference + annotations), heuristics, call-site tracing → the `possible`-tier edges that make "report, don't rule" earn its keep on dynamic code. Also `.pyi`/typeshed **stubs** (opt-in, scoped) and **resolution metrics** — coverage + confidence/source distribution, derived in-process and asserted in the golden eval.
