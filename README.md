...
- **P2 (in progress):** the value-typed resolver stack — the `possible`-tier edges that make "report, don't rule" earn its keep. **Landed:** `self.m()` → the enclosing class's member (+ cross-file inherited), and annotation-typed receivers (`u: User` → `User.save`) — `MEDIUM`/possible, since a subclass may override at runtime. **Next:** **relative-import resolution** (`from ..m import X`, currently deferred — it gates cross-module binding *and* annotation types on relative-import code); local inference (`x = User()`); heuristics; call-site tracing; `.pyi`/typeshed **stubs** (opt-in); and **resolution metrics** (coverage + confidence/source distribution, asserted in the golden eval).
