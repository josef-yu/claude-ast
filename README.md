...
- **P2 (in progress):** the value-typed resolver stack — the `possible`-tier edges that make "report, don't rule" earn its keep. **Landed:** `self.m()` → the enclosing class's member (+ cross-file inherited); annotation-typed receivers (`u: User` → `User.save`); local **construction inference** (`x = User(); x.save()`); **relative-import resolution** (`from ..model import X`); and **package re-export resolution** (`from pkg import X` follows `pkg/__init__` to the real defining module). All value-typed edges are `MEDIUM`/possible. and **resolution metrics** — `claude-ast index` now reports coverage + the definite/possible and per-source split (e.g. its own `src/`: 44% of refs bound, 212 definite / 23 possible), the measurement loop for the resolver stack. **Next:** the **call-site resolver + confidence merge** (a call-site observation corroborating an annotation escalates the edge to `definite`); a **builtins** pass and name-match **heuristics** for the large unbound tail the metrics expose; and `.pyi`/typeshed **stubs** (opt-in).
